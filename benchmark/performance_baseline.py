"""
Pale performance baseline — absolute overhead and scaling.
...
Three measurements:
  ...
  1. Absolute overhead — how many ms does Pale add per save and load?
     Targets:
       save < 500ms for models under 100MB
       load < 200ms for models under 100MB

  2. Scaling — does save time grow linearly with model size?
     Quadratic behavior in registry writes or CAS would show up here.

  3. No-op fast path — how fast is a save when nothing changed?
     Should be near-zero: only hashing + registry query, no CAS writes.

Methodology:
  - Each save rep uses a fresh store (cold, no no-op fast path for measurement 1/2).
  - Load reps reuse the same store — OS page cache is warm, matching real usage.
  - One warmup rep before timing to exclude import and JIT overhead.
  - Median of N reps reported.
"""
...
def _xgboost_model(n_rounds: int):
    ...
    return xgb.train(
        ...
        dtrain, num_boost_round=n_rounds, verbose_eval=False,
    )
...
def _save_fresh(adapter, model: Any, max_workers: int = 8) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pale_rep_") as rep_tmp:
        with PaleStore(root=Path(rep_tmp), run_id="r", adapter=adapter,
                       max_workers=max_workers) as store:
            store.save(model, step=1)


# ── measurement 1: absolute overhead ─────────────────────────────────────────

def measure_absolute(name: str, model: Any, adapter, n_reps: int = 10,
    ...
    with tempfile.TemporaryDirectory(prefix="pale_load_") as tmp:
        with PaleStore(root=Path(tmp), run_id="r", adapter=adapter,
                       max_workers=max_workers) as store:
            store.save(model, step=1)
...
# ── measurement 2: scaling ────────────────────────────────────────────────────

def measure_scaling(name: str, models_by_size: list, size_labels: list, adapter,
                    n_reps: int = 5, max_workers: int = 8) -> None:
    ...
    for model, label in zip(models_by_size, size_labels):
        size_str = _fmt_size(_model_size_bytes(model, adapter))
        try:
            save_ms = _median_ms(lambda m=model: _save_fresh(adapter, m, max_workers), n_reps)
...
# ── measurement 3: no-op fast path ────────────────────────────────────────────

def measure_noop(name: str, model: Any, adapter, n_reps: int = 10,
                 max_workers: int = 8) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pale_noop_") as tmp:
        with PaleStore(root=Path(tmp), run_id="r", adapter=adapter,
                       max_workers=max_workers) as store:
            store.save(model, step=1)
            noop_ms = _median_ms(lambda: store.save(model, step=_next_step(store)), n_reps)

    print(f"    noop save: {noop_ms:.1f}ms  (should be near-zero — hashing only, no CAS writes)")
...
if __name__ == "__main__":
    ...
    if "sklearn" in args.frameworks:
        ...
        print("\n[2] Scaling")
        measure_scaling("GBM", [_sklearn_model(n) for n in [10, 25, 50, 100]],
                        ["10 trees", "25 trees", "50 trees", "100 trees"],
                        sk_adapter, n_reps=max(3, args.reps // 2))
    ...
    if "xgboost" in args.frameworks:
        ...
        print("\n[1] Absolute overhead")
        measure_absolute("Booster 50 rounds", xgb_model, xgb_adapter, args.reps,
                         max_workers=1)

        print("\n[2] Scaling")
        measure_scaling("Booster", [_xgboost_model(n) for n in [10, 25, 50, 100]],
                        ["10 rounds", "25 rounds", "50 rounds", "100 rounds"],
                        xgb_adapter, n_reps=max(3, args.reps // 2), max_workers=1)

        print("\n[3] No-op fast path")
        measure_noop("Booster 50 rounds", xgb_model, xgb_adapter, args.reps,
                     max_workers=1)
    ...
    if "pytorch" in args.frameworks:
        ...
        measure_scaling(
            "MLP",
            [_pytorch_state_dict(hidden=h, n_layers=l) for h, l in
