...
def _default_run_id() -> str:
    return f"sklearn-logreg-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
...
class ExperimentConfig:
    ...
    random_state: int = 42
    max_iter: int = 1_000
    train_start_size: int = 400
    train_size_increment: int = 250
    n_steps: int = 5
    restore_step: int = 3
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(
        description="Run a checkpointed sklearn logistic regression experiment with tensorcas."
    )
    ...
    parser.add_argument("--train-start-size", type=int, default=400)
    parser.add_argument("--train-size-increment", type=int, default=250)
    ...
    return ExperimentConfig(
        ...
        train_start_size=args.train_start_size,
...
def validate_config(config: ExperimentConfig) -> None:
    ...
    if config.train_start_size <= 0:
        raise ValueError("train_start_size must be positive")
    if config.train_size_increment <= 0:
        raise ValueError("train_size_increment must be positive")
    if config.n_steps <= 0:
...
def training_size_for_step(config: ExperimentConfig, step: int) -> int:
    return config.train_start_size + (step - 1) * config.train_size_increment
...
def print_config(config: ExperimentConfig) -> None:
    ...
    print(f"train_start_size    : {config.train_start_size}")
    print(f"train_size_increment: {config.train_size_increment}")
...
def print_step_table(step_metrics: list[StepMetrics]) -> None:
    ...
    print("-" * 80)
    print(
        f"{'step':>4}  {'n_train':>8}  {'train_acc':>10}  {'valid_acc':>10}  {'test_acc':>10}"
    )
    for metrics in step_metrics:
        print(
            f"{metrics.step:>4}  {metrics.train_size:>8}  "
...
def train_experiment(config: ExperimentConfig) -> None:
    ...
    max_train_size = training_size_for_step(config, config.n_steps)
    if max_train_size > len(data.X_train_pool):
        raise ValueError(
            "Configured training schedule exceeds available training pool size"
        )

    step_metrics: list[StepMetrics] = []
    step_templates: dict[int, LogisticRegression] = {}
